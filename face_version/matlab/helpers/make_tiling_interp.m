...
V_flat_rots(1,:,:) = V_flat;
V_flat_rots(2,:,:) = V_flat;
V_flat_rots(2,:,2) = -V_flat_rots(2,:,2);
V_flat_rots(3,:,:) = V_flat_rots(2,:,:);
V_flat_rots(3,:,1) = -V_flat_rots(3,:,1);
V_flat_rots(4,:,:) = V_flat;
V_flat_rots(4,:,1) = -V_flat_rots(4,:,1);
...
for j=1:size(xs,2)
    ...
    for i=1:4
        ...
        ang = (1-i)*pi/2;
        %if i>2
        V_flat_rot = squeeze(V_flat_rots(i,:,:))';
        %else
