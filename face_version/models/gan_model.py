...
class GANModel(BaseModel):
    ...
    def build_model(self):
        with tf.device('/cpu:0'):
            ...
            # data and noise
            (real, pair) = self.dataLoader.get_next()
            real_charts = tf.reshape(real, [self.config.batch_size, self.config.depth_dim, self.config.spatial_dim, self.config.spatial_dim])
            pair = tf.reshape(pair, [self.config.batch_size//2, 2, self.config.depth_dim, self.config.spatial_dim, self.config.spatial_dim])
            pair_charts_1 = []
            pair_charts_2 = []
            # shuffling the pairwise comparisons
            label = np.ones((self.config.batch_size//2))
            for i in range(self.config.batch_size//2):
                if random.randint(0,1):
                    label[i] = 0
                    pair_charts_1.append(pair[i][1])
                    pair_charts_2.append(pair[i][0])
                else:
                    pair_charts_1.append(pair[i][0])
                    pair_charts_2.append(pair[i][1])  
            
            pair_charts = tf.stack([tf.stack(pair_charts_1), tf.stack(pair_charts_2)])
            # post process raw batch
            self.real_charts = self.process_batch(real_charts)
            self.pair_charts = self.process_batch(pair_charts)
            self.label_charts = tf.convert_to_tensor(tf.cast(label, tf.float32))

            # split to towers
            batches = tf.split(self.real_charts, self.config.n_gpus)
            pair_batches = tf.split(self.pair_charts, self.config.n_gpus)
            labels = tf.split(self.label_charts, self.config.n_gpus)
            self.noise = tf.random_normal([self.config.batch_size, self.config.latent_vec_dim])
            self.rank = tf.random_uniform([self.config.batch_size, 1], minval=-1, maxval=1)
            noises = tf.split(self.noise, self.config.n_gpus)
            ranks = tf.split(self.rank, self.config.n_gpus)

            # define generator and discriminator on CPU
            self.generator(self.noise, self.rank)
            ...
            self.rank_fake_cost = []
            self.fake_charts = []
            for device_index, (device, batch, pair_batch, label, noise, rank) in enumerate(zip(self.config.devices, batches, pair_batches, labels, noises, ranks)):
                with tf.device(device):
                    with tf.name_scope('tower.{0}'.format(device_index)) as scope:

                        disc_cost, gen_cost, rank_cost, rank_fake_cost, fake_charts = self.build_tower(noise, rank, batch, pair_batch, label)
                        self.disc_cost.append(disc_cost)
                        self.gen_cost.append(gen_cost)
                        self.fake_charts.append(fake_charts)
                        self.rank_cost.append(rank_cost)
                        self.rank_fake_cost.append(rank_fake_cost)
                        if device_index == 0:
            ...
            self.rank_cost = tf.add_n(self.rank_cost)/self.config.n_gpus
            self.rank_fake_cost = tf.add_n(self.rank_fake_cost)/self.config.n_gpus
    ...
    def process_batch(self, real_charts):
        ...
        if self.config.normalize_charts:
            # normalize charts
            real_charts = tf.subtract(real_charts, tf.reduce_mean(real_charts, axis=[2, 3], keep_dims=True))
    ...
    def build_tower(self, noise, rank, real_charts, pair_charts, label_charts):
        ...
        fake_charts = self.generator(noise, rank)
        disc_fake, rank_fake = self.discriminator(fake_charts)
        disc_real, _ = self.discriminator(real_charts)
        _, rank_real = self.discriminator(pair_charts) 

        gen_cost = -tf.reduce_mean(disc_fake)
        disc_cost = tf.reduce_mean(disc_fake) - tf.reduce_mean(disc_real)
        
        rank = tf.squeeze(rank)
        label_charts = tf.cast(label_charts, tf.float32)
        
        rank_cost = tf.losses.sigmoid_cross_entropy(logits=(rank_real[:tf.shape(rank_real)[0]//2]-rank_real[tf.shape(rank_real)[0]//2:]), multi_class_labels=label_charts)
       
        
        label_fake = tf.greater_equal(rank[:tf.shape(rank)[0]//2], rank[tf.shape(rank)[0]//2:])
        label_fake = tf.cast(label_fake, tf.float32)
        
        rank_fake_cost = tf.losses.sigmoid_cross_entropy(logits=(rank_fake[:tf.shape(rank_fake)[0]//2]-rank_fake[tf.shape(rank_fake)[0]//2:]), multi_class_labels=label_fake)        
        ...
        gen_cost += rank_fake_cost

        return disc_cost, gen_cost, rank_cost, rank_fake_cost, fake_charts
    ...
    def generator(self, noise, rank):

        if self.config.model == 'single_chart':
            return self.generator_single_chart(noise, rank)
        else:
            with tf.variable_scope("generator", reuse=tf.AUTO_REUSE):
                depths = self.config.depths
                spatial = self.config.spatial
                output = layers.linear.Linear('input_FC', self.config.latent_vec_dim + 1, (spatial[0]**2)*depths[0], tf.concat([noise, rank],1))
    ...
    def generator_single_chart(self, noise, rank):
    ...
    def discriminator(self, inputs):
        ...
        if self.config.model == "single_chart":
            ...
            with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
                ...
                rank_loss = tf.reshape(layers.linear.Linear('output_rank_FC', depths[ind + 1], 1, output), [-1])

            return total_loss, rank_loss
    ...
    def get_alignment_params(self):
        ...
        # load matlab files
        mat_content = sio.loadmat(os.path.join(self.config.database_dir, 'single', 'triplets_mat.mat'))
        self.triplets_mat = tf.cast(mat_content['triplets_mat'].toarray(), tf.float32)

        mat_content = sio.loadmat(os.path.join(self.config.database_dir, 'single', 'triplets_mat_reg.mat'))
        self.triplets_mat_reg = tf.cast(mat_content['triplets_mat'].toarray(), tf.float32)

        mat_content = sio.loadmat(os.path.join(self.config.database_dir, 'single', 'data_std.mat'))
        self.STD = mat_content['STD']

        mat_content = sio.loadmat(os.path.join(self.config.database_dir, 'single', 'gather_indices_p.mat'))
        self.gather_indices_p = mat_content['gather_indices_p']

        mat_content = sio.loadmat(os.path.join(self.config.database_dir, 'single', 'gather_indices_m.mat'))
        self.gather_indices_m = mat_content['gather_indices_m']

        mat_content = sio.loadmat(os.path.join(self.config.database_dir, 'single', 'scatter_indices_p.mat'))
        self.scatter_indices_p = mat_content['scatter_indices_p']

        mat_content = sio.loadmat(os.path.join(self.config.database_dir, 'single', 'scatter_indices_m.mat'))
        self.scatter_indices_m = mat_content['scatter_indices_m']

        mat_content = sio.loadmat(os.path.join(self.config.database_dir, 'single', 'triplets_masks.mat'))
