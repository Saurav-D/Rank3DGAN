...
def toric_symmetry(inputs, spatial_dim, data_format='NCHW', pooling='max', method='slice'):
    ...
    inputs_90 = flip_ud(inputs_pad)
    inputs_180 = flip_lr(inputs_90)
    inputs_270 = flip_ud(inputs_180)
...
def flip_ud(inputs):
    inputs_flip = tf.map_fn(lambda Input: tf.image.flip_up_down(Input), inputs)
    return inputs_flip

def flip_lr(inputs):
    inputs_flip = tf.map_fn(lambda Input: tf.image.flip_left_right(Input), inputs)
    return inputs_flip
...
def project_ST(model, inputs, gamma=None, data_format='NCHW'):
    ...
    mean_masks = tf.divide(tf.reduce_sum(friends_mat, [1,2], keep_dims=True),  # num_ucones x 1 x 1 x 3(xyz) x in_N
                           tf.reduce_sum(triplets_masks, [1,2], keep_dims=True)  # num_ucones x 1 x 1 x 1 x 1
